...
object HistoryStore {
    ...
    private fun sessionJson(id: String, title: String, messages: List<ChatMessage>): JSONObject {
        ...
        // Attached screenshots are NOT persisted to on-device history —
        // they're only meant to travel with the single message they were
        // attached to; keeping them out of every subsequent history save
        // avoids silently bloating SharedPreferences over time.
        messages.forEach { msgsArr.put(it.toJson(includeImage = false)) }

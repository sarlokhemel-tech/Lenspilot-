...
import com.hemel.lenspilot.ImagePickTrampolineActivity
import com.hemel.lenspilot.PendingImagePick
import com.hemel.lenspilot.Prefs
import com.hemel.lenspilot.R
import com.hemel.lenspilot.audio.LocalTts
import com.hemel.lenspilot.audio.RemoteTts
import com.hemel.lenspilot.chat.WorkflowStepPlan
...
class LenspilotAccessibilityService : AccessibilityService() {
    ...
    // Literal on-screen target label from the planner (WorkflowPreview.
    // targetLabel), forwarded unchanged to /api/analyze-screen on every
    // hop so the server can try a free local string-match before an LLM
    // call — see app.py's find_local_element_match. Empty = server just
    // reasons over the full tree every hop, same as before this existed.
    private var workflowTargetLabel: String = ""
    // "super_1_2" (default) or "super_lite" — see WorkflowPreview.system.
    // Super Lite has a REAL ordered step script (workflowSteps) instead of
    // the fresh-brain-every-screen loop below; see triggerSuperLiteStep().
    private var workflowSystem: String = "super_1_2"
    private var workflowSteps: List<WorkflowStepPlan> = emptyList()
    private var workflowStepIndex: Int = 0
    // False until the first Super Lite step has actually been sent once —
    // distinguishes "just started, run step 0" from "a screen change came
    // in, the user probably just tapped what we highlighted, advance".
    private var superLiteStepStarted: Boolean = false
    // Whether the LAST Super Lite analyze-screen call actually found
    // something to highlight. Only advance workflowStepIndex when true —
    // if nothing matched, the screen change that re-triggered us might be
    // unrelated (e.g. a keyboard popping up), so retry the SAME step
    // instead of skipping it.
    private var lastSuperLiteStepMatched: Boolean = true
    // How many consecutive misses the CURRENT step has had. Reset to 0
    // whenever a step matches (see triggerWorkflowStep's super_lite
    // branch). Past SUPER_LITE_MAX_STEP_RETRIES, triggerSuperLiteStep()
    // gives up on this step and reports an error instead of retrying
    // forever — a step whose `keys` never actually appear on any real
    // screen (bad plan, or the app's UI just doesn't match what the
    // planner guessed) would otherwise loop silently until Stop is tapped.
    private var superLiteStepRetryCount: Int = 0
    ...
    companion object {
        ...
        // Consecutive misses allowed on one Super Lite step before giving
        // up on it — see triggerSuperLiteStep().
        private const val SUPER_LITE_MAX_STEP_RETRIES = 6
    ...
    fun analyzeAndHighlight(
        ...
        // "super_1_2" (default) or "super_lite" — see WorkflowPreview.system.
        // `stepKeys` is only meaningful for a super_lite CLICK/INPUT/SCROLL
        // step (WorkflowStepPlan.keys) — forwarded as `keys` so the server
        // can try/extend its local match before its small executor model.
        system: String = "super_1_2",
        stepKeys: List<String> = emptyList(),
        // WorkflowStepPlan.type ("CLICK"/"INPUT"/"SCROLL"/"SYSTEM_INTENT"/
        // "DEEP_LINK") — only meaningful for super_lite. Tells the small
        // executor model whether to look for an on-screen element or to
        // decide a real open_app/open_settings action itself (it can see
        // the screen; the big planner that wrote this step couldn't).
        stepType: String? = null,
        // Optional photo the user attached from the mid-workflow message
        // input (see showMessageInput/overlayMsgAttachButton) — sent
        // straight through to /api/analyze-screen as image_base64,
        // alongside the normal accessibility-tree elements, so the AI can
        // actually look at whatever the user is pointing a camera/gallery
        // pick at ("এই সমস্যাটা দেখো") instead of only the current screen.
        ...
        if (elements.isEmpty() && retriesLeft > 0) {
            ...
            mainHandler.postDelayed(
                { analyzeAndHighlight(userGoal, contextHistory, targetLabel, system, stepKeys, stepType, imageBase64, onResult, onError, retriesLeft - 1) },
                350
            )
            return
        }
        ...
        val body = JSONObject().apply {
            ...
            if (!targetLabel.isNullOrBlank()) put("target_label", targetLabel)
            put("system", system)
            if (stepKeys.isNotEmpty()) put("keys", org.json.JSONArray(stepKeys))
            if (!stepType.isNullOrBlank()) put("type", stepType)
            if (contextHistory.isNotEmpty()) {
                put("context_history", org.json.JSONArray(contextHistory))
            }
            if (!imageBase64.isNullOrBlank()) put("image_base64", imageBase64)
            com.hemel.lenspilot.Prefs.userInfoJsonArrayOrNull(this@LenspilotAccessibilityService)
                ?.let { put("user_info", it) }
        }
            ...
            val result = ApiClient.streamAuthed(
                ...
                when (evt.optString("type")) {
                    ...
                    "done" -> {
                        ...
                        lastSuperLiteStepMatched = (resultObj.optJSONArray("highlights")?.length() ?: 0) > 0
    ...
    private fun applyAnalyzeScreenResult(
        ...
        if (!newGoal.isNullOrBlank() && workflowActive) {
            ...
            // Same run continuing under a refined goal — keep whatever
            // one-time data was attached to it, don't reset to empty.
            WorkflowContext.start(newGoal, WorkflowContext.oneTimeUserInfo)
    ...
    fun startWorkflow(
        ...
        targetLabel: String? = null,
        system: String = "super_1_2",
        steps: List<WorkflowStepPlan> = emptyList(),
        extraUserInfo: List<com.hemel.lenspilot.userinfo.UserInfoEntry> = emptyList(),
        ...
        workflowTargetLabel = targetLabel ?: ""
        workflowSystem = system
        workflowSteps = steps
        workflowStepIndex = 0
        superLiteStepStarted = false
        lastSuperLiteStepMatched = true
        superLiteStepRetryCount = 0
        lastGuidanceText = ""
        onWorkflowUpdate = onUpdate
        onWorkflowError = onError
        lastSeenWindowKey = null
        checkInFlight = false
        WorkflowContext.start(goal, extraUserInfo)
    ...
    /**
     * Guidance narration. Tries [RemoteTts] first (Edge TTS by default,
     * admin-switchable to Gemini TTS — see /api/tts + call_learning_tts()
     * in app.py) for a consistent voice across every device, since the
     * on-device engine's quality/reliability depends entirely on whatever
     * voice pack happens to be installed on THAT phone (see LocalTts's own
     * file doc for the history of chasing that). Falls back to on-device
     * [LocalTts] the moment RemoteTts reports any error (no network,
     * upstream failure, playback issue) so a network hiccup never leaves
     * guidance completely silent — LocalTts's own internal retry (busy-
     * engine reset + one fresh attempt) is still the last line of defense
     * after that.
     */
    private fun speakGuidance(text: String, onDone: (() -> Unit)? = null) {
        ...
        val baseUrl = getString(R.string.space_base_url)
        RemoteTts.speak(
            applicationContext, baseUrl, text,
            onDone = onDone,
            onError = { LocalTts.speak(applicationContext, text, onDone = onDone) }
        )
    }
    ...
    /**
     * Quick Settings tile's OTHER mode — used INSTEAD of
     * quickScanCurrentScreen() when Prefs.translateEnabled is on (see
     * QuickScanTrampolineActivity, which branches between the two).
     * Deliberately bypasses the whole workflow/analyze-screen guidance
     * loop: this just reads every text label currently on screen (via
     * the same accessibility-tree walk captureScreenElements() already
     * uses, so no extra permission and no MediaProjection screenshot
     * needed — node.text/contentDescription IS the real on-screen text,
     * far more reliable for Bengali than on-device OCR, see TextOcr's
     * kdoc), sends it to /api/translate as plain text, and speaks +
     * toasts the result. One-shot — no highlights, no workflow state.
     */
    fun translateCurrentScreen() {
        val labels = captureScreenElements()
            .map { it.label }
            .filter { it.isNotBlank() }
            .distinct()
        val screenText = labels.joinToString("\n").take(4000)
        if (screenText.isBlank()) {
            speakGuidance("স্ক্রিনে অনুবাদ করার মতো কোনো লেখা পাইনি।")
            return
        }
        val targetLang = Prefs.translateTargetLang(applicationContext)
        val baseUrl = getString(R.string.space_base_url)
        val body = JSONObject().apply {
            put("text", screenText)
        ...
        scope.launch {
            ...
            val result = ApiClient.streamAuthed(applicationContext, baseUrl, "/api/translate", body) { event ->
                when (event.optString("type")) {
                    "done" -> translated = event.optString("translated_text")
                    "error" -> Log.w(TAG, "translate error: ${event.optString("error")}")
                }
            }
            val text = translated?.trim()
            if (result.isFailure || text.isNullOrBlank()) {
                speakGuidance("স্ক্রিনের লেখা অনুবাদ করা যায়নি।")
                return@launch
            }
     ...
     */
    /**
     * Runs ONE step of a "Lenspilot Super Lite" plan (workflowSteps) — the
     * step-script counterpart to triggerWorkflowStep()'s fresh-brain loop.
     * `keys` (e.g. ["app_search:bKash"]) has its "prefix:" stripped before
     * being sent as match candidates — see analyze_screen's system=
     * "super_lite" branch in app.py for how those get tried (local match
     * first, then the small executor model) against the real screen.
     *
     * NOTE: SYSTEM_INTENT/DEEP_LINK steps (type field) are NOT special-cased
     * here yet — they go through the same analyze-screen call as CLICK
     * steps, matching the step's key against whatever's on screen (which
     * works fine for e.g. a home-screen app icon whose label matches, but
     * doesn't yet get the "skip the backend entirely, fire an Intent
     * directly" zero-cost path the plan schema was designed to enable).
     * That's a follow-up, not a correctness bug — worst case it costs one
     * extra local-match/executor call instead of zero.
     */
    private fun triggerSuperLiteStep() {
        val step = workflowSteps.getOrNull(workflowStepIndex)
        if (step == null) {
            onWorkflowUpdate?.invoke("কাজ শেষ হয়েছে!", true)
            stopWorkflow()
            return
        }
        if (superLiteStepRetryCount > SUPER_LITE_MAX_STEP_RETRIES) {
            // This step's keys never matched anything after several real
            // screen changes — the plan's guess for this step doesn't
            // match the app's actual UI. Stop instead of looping forever.
            onWorkflowError?.invoke("এই ধাপটা (${step.goal}) খুঁজে পাচ্ছি না — একটু ম্যানুয়ালি এগিয়ে দিন, বা আবার চেষ্টা করুন।")
            stopWorkflow()
            return
        }
        val matchKeys = step.keys.map { it.substringAfter(":") }.filter { it.isNotBlank() }
        analyzeAndHighlight(
            userGoal = step.goal,
            ...
            stepKeys = matchKeys,
            stepType = step.type,
            onResult = { guidance, _ -> onWorkflowUpdate?.invoke(guidance, false) },
            onError = { msg -> onWorkflowError?.invoke(msg) }
        )
    }

    fun triggerWorkflowStep(extraContext: String? = null, extraImageBase64: String? = null) {
        if (!workflowActive) return
        if (workflowSystem == "super_lite") {
            // Advance PAST the step we last highlighted, but only if it
            // actually found something to highlight — see
            // lastSuperLiteStepMatched's doc for why. First-ever call for
            // this workflow (superLiteStepStarted still false) always
            // runs step 0 without advancing. A miss bumps the retry
            // counter instead (see SUPER_LITE_MAX_STEP_RETRIES / triggerSuperLiteStep).
            if (superLiteStepStarted) {
                if (lastSuperLiteStepMatched) {
                    workflowStepIndex++
                    superLiteStepRetryCount = 0
                } else {
                    superLiteStepRetryCount++
                }
            }
            superLiteStepStarted = true
            triggerSuperLiteStep()
            return
        }
        ...
        analyzeAndHighlight(
            ...
            targetLabel = workflowTargetLabel.ifBlank { null },
            imageBase64 = extraImageBase64,
            onResult = { guidance, taskComplete ->
    ...
    private fun showMessageInput() {
        ...
        val previewRow = view.findViewById<View>(R.id.overlayMsgImagePreviewRow)
        val previewImage = view.findViewById<ImageView>(R.id.overlayMsgImagePreview)
        var pendingImageBase64: String? = null

        val params = WindowManager.LayoutParams(
            WindowManager.LayoutParams.WRAP_CONTENT,
            WindowManager.LayoutParams.WRAP_CONTENT,
            // ROOT-CAUSE FIX for "কিবোর্ড চালু হয় না": this used to be
            // TYPE_ACCESSIBILITY_OVERLAY, same as the pass-through
            // highlight overlay. That type is meant for
            // draw-only/pass-through accessibility visuals — on most
            // OEM builds WindowManagerService simply never routes IME
            // focus to it, no matter what flags/softInputMode are set,
            // so requestFocus()+showSoftInput() below silently did
            // nothing. FallbackGuideService's identical card already
            // used TYPE_APPLICATION_OVERLAY and never had this problem
            // (see that file) — SYSTEM_ALERT_WINDOW is already required
            // and checked in MainActivity before any workflow can start,
            // so it's safe to use here too, just for this one focusable
            // input card (the control bar / highlight overlay stay on
            // TYPE_ACCESSIBILITY_OVERLAY, unaffected).
            ...
            // ADJUST_RESIZE does nothing useful for a WRAP_CONTENT floating
            // window (nothing to resize), and on some OEMs it's exactly
            // what suppressed the IME from ever being requested. PAN +
            // forcing the IME visible the moment this window gets focus
            // is the reliable combination for a small floating input card.
        ...
        fun send() {
            ...
            if (workflowActive && !checkInFlight) {
                triggerWorkflowStep(
                    extraContext = text.ifBlank { "(ছবি পাঠিয়েছে, লেখেনি — ছবিটা দেখে বুঝে নাও)" },
                    extraImageBase64 = image
                )
            }
        }

        // × — closes the card outright, with nothing sent, whether or not
        // the user has typed/attached anything. This is the fix for "খুলে
        // গেলে বন্ধ করার উপায় নেই যদি লিখতে না চাই".
        view.findViewById<ImageButton>(R.id.overlayMsgCloseButton).setOnClickListener { hideMessageInput() }

        // 📎 — hands off to the invisible trampoline Activity (a Service
        // can't launch a picker or get its result directly, see
        // PendingImagePick) to pick a photo from the gallery, then shows a
        // small thumbnail chip above the input so the user can see/remove
        // it before sending.
        view.findViewById<ImageButton>(R.id.overlayMsgAttachButton).setOnClickListener {
            PendingImagePick.awaitNext { base64 ->
                mainHandler.post {
                    if (messageInputView !== view) return@post  // card closed meanwhile
                    if (base64 == null) return@post
                    ...
                    } catch (e: Exception) { /* ignore a bad/undecodable pick */ }
                }
            }
            startActivity(
                Intent(this, ImagePickTrampolineActivity::class.java)
                    .addFlags(Intent.FLAG_ACTIVITY_NEW_TASK)
            )
        }
        view.findViewById<ImageButton>(R.id.overlayMsgImageRemoveButton).setOnClickListener {
        ...
        requestKeyboardFor(input)
    }

    /** Belt-and-braces keyboard trigger for a just-added focusable
     * overlay window: input.post{} alone races the window actually
     * receiving focus from WindowManagerService, so on slower devices
     * showSoftInput() could fire before the window is focused and get
     * silently dropped. Also listening for the window-focus-gained
     * callback and re-requesting there catches that case without
     * duplicating the call once focus is already in (SHOW_IMPLICIT is a
     * no-op if the keyboard is already up). */
    private fun requestKeyboardFor(input: EditText) {
        fun show() {
            input.requestFocus()
        ...
        show()
        input.post { show() }
        input.viewTreeObserver.addOnWindowFocusChangeListener(object : android.view.ViewTreeObserver.OnWindowFocusChangeListener {
            override fun onWindowFocusChanged(hasFocus: Boolean) {
                if (hasFocus) {
                    show()
                    input.viewTreeObserver.removeOnWindowFocusChangeListener(this)
                }
            }
        })
    }

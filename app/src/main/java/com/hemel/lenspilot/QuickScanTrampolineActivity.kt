...
class QuickScanTrampolineActivity : Activity() {
    override fun onCreate(savedInstanceState: Bundle?) {
        ...
        Handler(Looper.getMainLooper()).postDelayed({
            // Two things this tile can do, picked by the keyboard's ⚙
            // translate toggle (Prefs.translateEnabled) — normal guidance
            // scan, or read-and-translate the whole screen. Same tile,
            // same permission, same trampoline; only the destination
            // function differs.
            if (Prefs.translateEnabled(applicationContext)) {
                Toast.makeText(applicationContext, "স্ক্রিনের লেখা অনুবাদ করছি...", Toast.LENGTH_SHORT).show()
                service.translateCurrentScreen()
            } else {

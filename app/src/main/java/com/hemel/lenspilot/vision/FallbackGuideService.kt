...
class FallbackGuideService : Service() {
    ...
    // Mirrors LenspilotAccessibilityService's field — see there for the
    // full explanation. Literal on-screen target label from the planner,
    // forwarded to /api/analyze-screen so the server can try a free
    // local string-match before an LLM call.
    private var workflowTargetLabel: String = ""
    // Mirrors LenspilotAccessibilityService's Super Lite state fields —
    // see there for the full explanation.
    ...
    fun analyzeAndHighlight(
        ...
        // Optional photo attached via the mid-workflow message input (see
        // LenspilotAccessibilityService.analyzeAndHighlight for the full
        // comment) — forwarded to /api/analyze-screen as image_base64.
        ...
        scope.launch {
            ...
            if (elements.isEmpty() && retriesLeft > 0) {
                checkInFlight = false
                mainHandler.postDelayed({ analyzeAndHighlight(userGoal, contextHistory, targetLabel, system, stepKeys, stepType, imageBase64, onResult, onError, retriesLeft - 1) }, 350)
                return@launch
            }
            if (elements.isEmpty()) {
                checkInFlight = false
    ...
    /** Mirrors LenspilotAccessibilityService.speakGuidance — see there for
     * the full explanation. Tries RemoteTts (Edge/Gemini via /api/tts)
     * first, falls back to on-device LocalTts on any error. */
    ...
    /** Mirrors LenspilotAccessibilityService.triggerSuperLiteStep — see
    ...
    private fun showMessageInput() {
        ...
        val params = WindowManager.LayoutParams(
            ...
            // See LenspilotAccessibilityService.showMessageInput() for why
            // PAN + STATE_ALWAYS_VISIBLE, not ADJUST_RESIZE, for a
            // WRAP_CONTENT floating card.
    ...
    /** See LenspilotAccessibilityService.requestKeyboardFor() — same
     * belt-and-braces fix, ported here so the fallback path's keyboard
     * is exactly as reliable as the accessibility-service path. */

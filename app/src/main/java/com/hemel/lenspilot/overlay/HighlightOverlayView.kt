...
class HighlightOverlayView(context: Context) : View(context) {
    ...
    override fun onDraw(canvas: Canvas) {
        super.onDraw(canvas)
        // This view lives inside a background AccessibilityService, not a
        // normal Activity — an uncaught exception during a draw pass (bad
        // StaticLayout width, a transient 0-size window right after
        // rotation, etc.) doesn't show an "app has stopped" dialog, it
        // just silently kills the whole service process, same crash class
        // documented in LenspilotAccessibilityService.captureScreenElements
        // and ensureOverlayView(). This runs on EVERY highlight render, so
        // it's guarded the same way: skip this one bad frame, never take
        // the service down over it.
        try {
            drawInternal(canvas)
        } catch (e: Exception) {
            android.util.Log.w("HighlightOverlayView", "onDraw failed, skipping this frame", e)
        }
    }

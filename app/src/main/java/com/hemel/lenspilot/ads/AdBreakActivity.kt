...
class AdBreakActivity : AppCompatActivity() {
    ...
    override fun onCreate(savedInstanceState: Bundle?) {
        ...
        lifecycleScope.launch {
            // Resolves AdMob vs Start.io from the server before the first
            // preload — see AdNetworkManager. Both SDKs are init'd inside
            // this call; nothing shown to the user changes either way.
            AdNetworkManager.init(this@AdBreakActivity, baseUrl)
            preloadAd()
        }
    }
    ...
    private fun preloadAd() {
        AdNetworkManager.load(this, adUnitId, uid = null)
    }

    private fun onWatchTapped() {
        if (!AdNetworkManager.isReady()) {
            watchButton.isEnabled = false
            watchButton.text = getString(R.string.ad_loading)
            AdNetworkManager.load(this, adUnitId, uid = null) { ready ->
                runOnUiThread {
